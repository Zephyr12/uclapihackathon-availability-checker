...
function test(entity,days){
    ...
    var frees = [];
    var eventi = getEventsI(entity, new Date().toISOString(), new Date(Date.now()+(days*24*60*60*1000)).toISOString());
    ...
    eventi.done(function(result){
        ...
        getTimeTable().done(function(timetable){
        timetable = timetable.data;
        timetable.sort(function (a, b){a[0] > b[0];});
        var arr = [];
        for (var i = 0; i < timetable.length; i++){
            arr.push({
              startime: new Date(timetable[i][0]),
              endtime: new Date(timetable[i][1])
            });
        }
